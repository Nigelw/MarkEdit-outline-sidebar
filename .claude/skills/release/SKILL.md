...
1. **Bump the version** in `package.json` to the new version (no `v` prefix). Edit the file
   directly — don't run `npm version`, which also creates a tag and would fight step 6.

2. **Typecheck**: `npm run typecheck`. Fix or report any errors before continuing.

3. **Build**: `npm run build`. This bakes the new `package.json` version into the bundle as
   `__EXTENSION_VERSION__`, writes `dist/markedit-outline.js`, and deploys a copy into the local
   MarkEdit scripts folder.

4. **Verify the bundle carries the new version**:
...
5. **Commit** both files together:
   `git add package.json dist/markedit-outline.js` then commit as
...
6. **Tag** the release commit: `git tag -a v<version> -m "v<version>"` (annotated tag).

7. **Push** the branch and the tag: `git push origin main` and `git push origin v<version>`.

8. **Publish the GitHub release** so `releases/latest` advances (the updater reads *latest*, so an
   unpublished tag alone won't trigger updates):
   `gh release create v<version> --title "v<version>" --generate-notes`
   Use `--generate-notes` for auto notes from commits, or draft notes with the user if they want
   something curated. Do **not** attach a binary asset — Method A serves the bundle from the raw
   tag path, not from release assets.

9. **Verify the updater's download URL resolves** (catches a missing/mis-committed `dist/`):
   ```
...
and the result of the step-9 URL check so they know the auto-updater will serve it.
...
- **Never tag without rebuilding.** The whole scheme relies on the committed `dist/` at the tag
  matching `package.json`'s version. Step 4 guards this.
