...
${JSON.stringify(e)}`,buttons:["OK"]})}async function lt(){const e=I(),t=await O();if(t===void 0){await N(e);return}const i=Array.isArray(t[c])?t[c]:[];if(i.some(L)){await a.MarkEdit.showAlert({title:"Toolbar button already configured",message:`A toolbar toggle for the Outline Sidebar is already in your settings.json.
...
${JSON.stringify(e)}`,buttons:["OK"]})}async function dt(){const e=await O();if(e===void 0){await N(I());return}const t=Array.isArray(e[c])?e[c]:[],i=t.filter(n=>!L(n));if(i.length===t.length){await a.MarkEdit.showAlert({title:"Nothing to remove",message:"No Outline Sidebar toolbar item was found in settings.json.",buttons:["OK"]});return}i.length===0?delete e[c]:e[c]=i;const o=await z(e);await a.MarkEdit.showAlert({title:o?"Toolbar button removed":"Failed to write settings.json",message:o?"Restart MarkEdit to apply. You can also remove it from the toolbar via View → Customize Toolbar….":"Could not write settings.json.",buttons:["OK"]})}function ct(e,t){a.MarkEdit.addMainMenuItem({title:"Outline Sidebar",children:[{title:g,key:e.shortcut.key,modifiers:e.shortcut.modifiers,action:()=>t.toggle(),state:()=>({isSelected:t.isOpen()})},{separator:!0},{title:"Add Toolbar Button to settings.json…",action:()=>void lt()},{title:"Remove Toolbar Button…",action:()=>void dt()}]})}const V=W(),u=new ot(V);ct(V,u);let f;a.MarkEdit.addExtension(S.EditorView.updateListener.of(e=>{e.docChanged?(f!==void 0&&clearTimeout(f),f=setTimeout(()=>u.refresh(),250)):e.selectionSet&&u.updateActive()}));let k=!1;function $(){k||(k=!0,u.mount(),u.shouldStartOpen()&&u.open())}a.MarkEdit.onEditorReady(()=>$());try{a.MarkEdit.editorView!==void 0&&$()}catch{}
