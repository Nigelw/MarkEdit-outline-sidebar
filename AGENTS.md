...
`dist/markedit-outline.js` is **committed on purpose** — the in-app updater downloads it straight from the tagged commit (see *Releases*), so it must be present in the repo.
...
- **The updater** ([`src/updater.ts`](src/updater.ts)) runs on `onAppReady` (throttled to once a day via `localStorage`) and on demand from the menu. It fetches `releases/latest` from the GitHub API, compares the release tag against the baked-in `__EXTENSION_VERSION__` (a small semver compare), and — per the `update` setting — installs silently, prompts, or does nothing. Installing downloads the built bundle from the release tag's raw path and overwrites the running script file (`__FILE_PATH__`) via `MarkEdit.createFile`. Skipped versions are remembered in `localStorage`. See the *Releases* section for the server-side contract this depends on.
...
Releases are cut with the **`release` skill** (`.claude/skills/release/SKILL.md`) — run `/release` (or ask Claude Code to "cut a release"). It bumps the version, updates the `CHANGELOG.md`, rebuilds, commits, tags `vX.Y.Z`, pushes, and publishes a GitHub release.

The updater relies on a "Method A" contract: it downloads the build straight from the tagged commit at `raw.githubusercontent.com/Nigelw/MarkEdit-outline-sidebar/refs/tags/vX.Y.Z/dist/markedit-outline.js`. For a release to be installable, **three things must agree at the tagged commit**:
...
3. The git tag is `vX.Y.Z` and points at the commit containing that rebuilt bundle.

The repo must stay **public** for the unauthenticated GitHub API / raw fetches to work. The skill enforces these invariants; see its steps and gotchas for details.
